...
import org.kexie.gradle.hotfix.workflow.beans.CloneMapping;
...
final class AdjustCloneClassTask extends Work<List<CloneMapping>, List<CtClass>> {
    ...
    doWork(ContextWith<List<CloneMapping>> context) {
        try {
            ...
            for (CloneMapping entry : context.getData()) {
                ...
                List<CtClass> part = adjustClass(context, source, clone);
                result.add(clone);
                result.addAll(part);
            }
    ...
    private List<CtClass> adjustClass(
            Context context,
            CtClass source,
            CtClass clone)
        ...
        for (CtMethod method : clone.getDeclaredMethods()) {
            ...
            List<CtClass> classes = adjustMethod(context, source, method);
            part.addAll(classes);
        }
    ...
    private List<CtClass> adjustMethod(
            Context context,
            CtClass source,
            CtMethod cloneMethod)
            throws CannotCompileException {
        MethodTransform transform = new MethodTransform(cloneMethod, source, context);
        cloneMethod.instrument(transform);
        return transform.getInnerClasses();
    }
    ...
    private final class MethodTransform extends ExprEditor {
        private final CtMethod current;
        private final CtClass source;
        private final Context context;
        private final List<CtClass> innerClasses = new LinkedList<>();

        List<CtClass> getInnerClasses() {
            return innerClasses;
        }
...
//                super.edit(c);
        ...
        public void edit(MethodCall m) throws CannotCompileException {
            try {
                ...
                boolean isAccessible = isAccessible(current.getDeclaringClass(), method);
                ...
                if (isDirect) {
                    ...
                    if (Modifier.isStatic(method.getModifiers())) {
                        if (isOverload) {
                            pramThis = current.getDeclaringClass().getName();
                        } else {
        ...
        public void edit(FieldAccess f) throws CannotCompileException {
            try {
                ...
                boolean isAccessible = isAccessible(current.getDeclaringClass(), field);
